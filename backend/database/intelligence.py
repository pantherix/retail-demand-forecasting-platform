...
from sqlalchemy import Column, Integer, Float, String, DateTime, Boolean, Text, JSON, ForeignKey, Index
...
class SearchFilter(Base):
    ...
    __table_args__ = (
        Index("ix_search_filters_user", "user_id"),
    )
...
class DataQualityReport(Base):
    ...
    __table_args__ = (
        Index("ix_dqr_portfolio_date", "portfolio_id", "report_date"),
    )
...
class DecisionAuditLog(Base):
    ...
    # Event
    event_type = Column(String(100), nullable=False)  # view, filter, export, acknowledge, action
...
class ExportRecord(Base):
    ...
    __table_args__ = (
        Index("ix_export_user_date", "user_id", "created_at"),
    )
...
class Recommendation(Base):
    ...
    # Lifecycle
    status = Column(String(50), default="active")  # active, implemented, rejected, expired
