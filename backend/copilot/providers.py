...
def execute_tool(func_name: str, func_args: dict) -> dict:
    ...
    try:
        if func_name == "get_forecast":
            from copilot.tools import get_forecast_tool
            ...
            from copilot.tools import get_inventory_health_tool
...
class OpenAIProvider(LLMProvider):
    ...
    def chat(
        ...
        if not self.api_key or self.api_key == "your-openai-key-here":
            raise ValueError("OpenAI API key is missing or placeholder.")
        ...
        try:
            ...
            client = OpenAI(api_key=self.api_key, timeout=2.0)
            ...
            if tool_calls and tools:
                ...
                final_content = second_response.choices[0].message.content.strip()
            else:
    ...
    def chat(
        ...
        try:
            ...
            if tool_calls and tools:
                ...
                # Remove tool parameters to generate raw response
                ...
                final_content = second_response.json()["choices"][0]["message"][
                    "content"
                ].strip()
            else:
                final_content = message_data["content"].strip()
    ...
    def chat(
        ...
        try:
            ...
            try:
                ...
                return {
                    "answer": final_content,
                    "insight": "Local LLM returned unstructured response.",
                    "confidence": "low",
                }
        except Exception as e:
