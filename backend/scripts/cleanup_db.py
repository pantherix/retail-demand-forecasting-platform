        ...
        for prod in valid_products:
            ...
            if avg_daily_sales <= 0:
                ...
                dynamic_safety_stock = float(
                    service_level_z * std_dev * math.sqrt(lead_time)
                )
                reorder_point = float(
            ...
            if risk:
                ...
                # Calculate reorder_qty first to use for revenue/profit at risk
                reorder_qty = 0.0
                if tot_stock < reorder_point:
                    annual_demand = (
                        float(f_sum * 12) if f_sum > 0 else 1.0
                    )  # Avoid division by zero
                    holding_cost = max(
                        float(prod.unit_cost) * 0.25, 0.5
                    )  # min 0.5 to avoid division by zero
                    eoq = (
                        math.sqrt((2 * annual_demand * 100.0) / holding_cost)
                        if holding_cost > 0
                        else 0.0
                    )
                    reorder_qty = max(
                ...
                # Now calculate revenue and profit at risk based on reorder_quantity
                risk.revenue_at_risk = float(reorder_qty * prod.base_price)  # type: ignore[reportAttributeAccessIssue]
                risk.profit_at_risk = float(reorder_qty * (prod.base_price - prod.unit_cost))  # type: ignore[reportAttributeAccessIssue]
