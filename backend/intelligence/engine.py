...
class FinancialImpactAnalyzer:
    ...
    def compute_impact(
        ...
        demand_days = int(demand_forecast / daily_usage) if daily_usage > 0 else 0
        ...
        if days_of_cover < 7:
            ...
            action = DecisionAction.INCREASE_ORDER.value if optimal_order_qty > 0 else DecisionAction.HOLD.value
            urgency_score = 0.75
            ...
            severity = DecisionSeverity.HIGH.value
            action = DecisionAction.REDUCE_ORDER.value if inventory_gap < 0 else DecisionAction.LIQUIDATE.value
            urgency_score = 0.70
            impact_drivers['excess_inventory'] = True
            risk_factors['excess_pct'] = excess_stock / current_stock if current_stock > 0 else 0
        elif revenue_at_risk > (retail_price * daily_usage * 5):  # >5 days potential lost sales
        ...
        if action == DecisionAction.ORDER_NOW.value or action == DecisionAction.INCREASE_ORDER.value:
            potential_loss = revenue_at_risk
        elif action == DecisionAction.REDUCE_ORDER.value or action == DecisionAction.LIQUIDATE.value:
...
class DecisionFeedGenerator:
    ...
    @staticmethod
    def _generate_description(financial_impact: Dict[str, Any], sku_profile: Dict[str, Any]) -> str:
        ...
        return descriptions.get(action, f"Inventory decision required for {sku_profile.get('sku_name', 'SKU')}")
...
class SKUIntelligenceEngine:
    ...
    def build_sku_profile(
        ...
        annual_revenue = total_revenue if len(sales_data) >= 365 else total_revenue * (365 / len(sales_data))
        ...
        # Financial metrics
        gross_margin_pct = ((financial_data.get('retail_price', 100) - 
                            financial_data.get('unit_cost', 50)) / 
                           financial_data.get('retail_price', 100) * 100)
        margin_contribution = annual_revenue * (gross_margin_pct / 100)
        
        # Inventory metrics
        inventory_turnover = annual_revenue / (inventory_data.get('avg_inventory_value', 1) or 1)
        ...
        forecast_volatility = forecast_data.get('volatility', np.std(quantities) if len(quantities) > 0 else 0)
        
        # Data quality assessment
        missing_pct = 1 - (sales_data['quantity'].notna().sum() / len(sales_data))
        outlier_pct = SKUIntelligenceEngine._detect_outliers_pct(sales_data['quantity'].values)
        ...
        health_components = [
            ...
            ((100 - (inventory_turnover * 10)) if inventory_turnover > 0 else 50, 0.10),  # Turnover 10%
            (100 if data_quality != DataQualityLevel.POOR.value else 50, 0.10),  # Data quality 10%
        ]
        
        overall_health_score = sum(score * weight for score, weight in health_components)
...
class RevenueAtRiskAnalyzer:
    ...
    def compute_portfolio_rar(
        ...
        total_potential_mitigation = (
            sum(si['potential_loss'] for si in sku_impacts) +
        ...
        # Top risks by SKU
        top_risks = sorted(
            [{"sku": si.get('sku', 'Unknown'), "revenue_at_risk": si['revenue_at_risk']}
        ...
        return {
            ...
            "recommended_actions_count": len([si for si in sku_impacts if si['action'] != DecisionAction.MONITOR.value]),
            "top_risks": top_risks,
