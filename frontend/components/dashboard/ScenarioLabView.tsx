...
export default function ScenarioLabView() {
  ...
  const selectedDatasetId = useStore((state) => state.selectedDatasetId);
  ...
  // Run simulation – now logs request/response and propagates real errors
  const runSimulation = async (
    ...
    setErrorMsg(null);
    try {
      console.log("Running simulation with payload", {
      ...
      setData({
        ...
        suggested_transfers: Array.isArray(res?.suggested_transfers)
          ? res.suggested_transfers
          : [],
        summary: res?.summary || null,
      });
    } catch (err: any) {
      console.error("Simulation error", err);
      setErrorMsg(err.message || err.detail || "Simulation matrix failed to compute.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Optimistic transfer execution – deep clone for safe rollback
  const handleExecuteTransfer = async (transfer: any) => {
    ...
    const originalData = data ? JSON.parse(JSON.stringify(data)) : null;

    // Optimistically remove the transfer from UI
    if (data?.suggested_transfers) {
      setData((prev: any) => ({
        ...prev,
        suggested_transfers: prev.suggested_transfers.filter(
          (t: any) => t.sku !== transfer.sku
        ),
      }));
    }
    ...
    try {
      ...
      addToast(
        `Stock transfer authorized under stress parameters. Shipped ${transfer.quantity.toLocaleString()} units.`,
        "success"
      );
      // Refresh both local simulation and global state
      runSimulation();
      triggerRefresh();
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto space-y-8 relative py-2">
      ...
      {!loading && data && (
        <>
          ...
          {/* Critical Transfer Recommendation */}
          {simulatedTransfer && (
            <div className="space-y-4">
