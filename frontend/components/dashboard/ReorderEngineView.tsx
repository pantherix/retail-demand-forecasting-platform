/* ReorderEngineView – Fixed version */
"use client";
...
export default function ReorderEngineView() {
  ...
  // Track which SKU is being approved to prevent duplicate clicks
  const [submittingSku, setSubmittingSku] = useState<string | null>(null);

  const fetchData = () => {
    setLoading(true);
    setErrorMsg(null);
    console.log("Fetching reorder data", { datasetId: selectedDatasetId });
    api
      .getReorder(selectedDatasetId || undefined)
      .then((res) => {
      ...
      .catch((err: any) => {
        ...
        setErrorMsg(err.message || err.detail || "Failed to load reorders list.");
        setLoading(false);
      });
  };
  ...
  // Optimistic approval of a reorder
  const handleApproveReorder = async (sku: string, qty: number) => {
    if (submittingSku) return;
    setSubmittingSku(sku);

    const originalReorders = JSON.parse(JSON.stringify(reorders)); // deep clone for rollback

    // Optimistically set recommended qty to 0 and increase stock
    setReorders((prev) =>
      prev.map((r) =>
        r.sku === sku ? { ...r, recommended_reorder_qty: 0, current_stock: r.current_stock + qty } : r
      )
    );
    ...
    try {
      ...
      setReorders(originalReorders); // rollback UI
      addToast(err.message || err.detail || "Reorder approval failed", "error");
    } finally {
  ...
  const topOrder = useMemo(() => activeOrders[0] ?? null, [activeOrders]);
        ...
        {reorders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {reorders.map((item) => (
              <div
                key={item.sku}
                className="bg-[#111114] border border-zinc-800 p-6 rounded-lg flex flex-col justify-between gap-4 hover:border-zinc-300 dark:hover:border-[#27272A] transition-colors shadow-sm"
              >
                <div className="space-y-3">
                  <div className="flex justify-between items-start">
                    <span className="font-mono text-xs font-bold text-zinc-450 dark:text-zinc-500">
                      {item.sku}
                    </span>
                    <span className="text-xs font-mono font-bold text-zinc-500">
                      Cover: <strong className={item.days_of_cover < 7 ? "text-rose-600 dark:text-rose-400 font-bold" : "text-zinc-800 dark:text-zinc-300 font-medium"}>{item.days_of_cover}d</strong>
                    </span>
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-zinc-955 dark:text-zinc-50 leading-snug">
                      {item.product_name}
                    </h3>
                  </div>
                  {!item.supplier_id && (
                ...
                {item.recommended_reorder_qty > 0 ? (
                  ...
                  >
                    {!item.supplier_id
                      ? "PO Disabled (No Supplier)"
                      : submittingSku === item.sku
                      ? "Approving..."
                      : `Approve Order (${item.recommended_reorder_qty})`}
                  </button>
                ) : (
