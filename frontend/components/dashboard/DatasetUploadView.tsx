...
export default function DatasetUploadView() {
  ...
  const executeImport = async () => {
    ...
    const runImport = async (
      ...
      return await api.importDataset({
        ...
        // NOTE: api.importDataset types only accept: { temp_file_id?, source_type, mapping?, confirm_low_confidence? }
        // Keep privacy confirmations for backend behavior via confirm_low_confidence.
        // If backend requires these flags, they must be added to api.ts typing.
      });
    };
  ...
  const triggerDatabaseCleanup = async () => {
    ...
    try {
      const res = await api.cleanupDataset();
      addToast(
        `Database sanitized! Removed ${res.metrics.products_removed} fake products, ${res.metrics.sales_removed} sales records. Warehouse ratios normalized.`,
        "success"
      );
      fetchDatasets();
    } catch (err: any) {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-zinc-900 font-mono text-zinc-350">
                  {datasets.map((d) => (
                    <tr key={d.id} className="hover:bg-[#121217] transition-all duration-150 group border-l-2 border-transparent hover:border-l-[#E10600]">
