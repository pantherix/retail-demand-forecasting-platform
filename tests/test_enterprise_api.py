...
def auth_headers():
    ...
    from database.intelligence import Base as IntelBase
    ...
    db = SessionLocal()
    existing = db.query(User).filter(User.username == "test_ent_user").first()
    if existing:
        db.delete(existing)
        db.commit()
    db.close()
...
def test_product_intelligence_sku(auth_headers):
    ...
    print("SKUS IN DB:", [p.sku for p in all_prods])
    db.close()
...
def test_alerting_system(auth_headers):
    ...
    if len(alerts) > 0:
        ...
        resolve_resp = client.post(
            f"/api/enterprise/alerts/{alert_id}/resolve",
            headers=auth_headers
        )
...
def test_role_based_financial_gating(auth_headers):
    ...
    # Get login headers
    planner_resp = client.post("/api/auth/login", data={"username": "planner_user", "password": "plannerpassword"})
    planner_headers = {"Authorization": f"Bearer {planner_resp.json()['access_token']}"}

    manager_resp = client.post("/api/auth/login", data={"username": "manager_user", "password": "managerpassword"})
    manager_headers = {"Authorization": f"Bearer {manager_resp.json()['access_token']}"}

    director_resp = client.post("/api/auth/login", data={"username": "director_user", "password": "directorpassword"})
    director_headers = {"Authorization": f"Bearer {director_resp.json()['access_token']}"}

    # 2. Create a high cost PO (total cost: ₹150,000)
    # Unit cost of SKU-101 is 60. Quantity of 2500 units makes it 150,000
    create_payload = {
        "supplier_id": 1,
        "items": [{"sku": "SKU-101", "quantity": 2500}]
    }
    po_resp = client.post("/api/enterprise/purchase-orders/create", json=create_payload, headers=planner_headers)
    ...
    submit_resp = client.post(f"/api/enterprise/purchase-orders/{po_id}/submit", headers=planner_headers)
    assert submit_resp.status_code == 200
    assert submit_resp.json()["status"] == "Pending Approval"

    # 4. Planner attempts to approve PO -> should fail 403
    app_plan = client.post(f"/api/enterprise/purchase-orders/{po_id}/approve", headers=planner_headers)
    assert app_plan.status_code == 403
    assert "do not have approval permissions" in app_plan.json()["detail"]

    # 5. Manager attempts to approve PO of 150,000 -> should fail 403 (limit is 100k)
    app_mgr = client.post(f"/api/enterprise/purchase-orders/{po_id}/approve", headers=manager_headers)
    ...
    app_dir = client.post(f"/api/enterprise/purchase-orders/{po_id}/approve", headers=director_headers)
...
def test_two_way_integration_service(auth_headers):
    ...
    app_resp = client.post(f"/api/enterprise/purchase-orders/{po_id}/approve", headers=director_headers)
    ...
    db = SessionLocal()
    from database.models import AuditLog
    shopify_po_log = db.query(AuditLog).filter(
        AuditLog.resource == f"PO {po_id}",
        AuditLog.action == "shopify_sync_po"
    ).first()
    assert shopify_po_log is not None
    assert "Shopify Order ID 89472938472" in shopify_po_log.detail

    zoho_po_log = db.query(AuditLog).filter(
        AuditLog.resource == f"PO {po_id}",
        AuditLog.action == "zoho_sync_po"
    ).first()
    ...
    trans_resp = client.post("/api/enterprise/transfers", json=transfer_payload, headers=director_headers)
    assert trans_resp.status_code == 200

    # 5. Check db logs for transfer sync
    from database.models import InventoryTransfer
    latest_transfer = db.query(InventoryTransfer).order_by(InventoryTransfer.id.desc()).first()
    assert latest_transfer is not None

    shopify_tr_log = db.query(AuditLog).filter(
    ...
    zoho_tr_log = db.query(AuditLog).filter(
        AuditLog.resource == f"Transfer {latest_transfer.id}",
...
def test_decision_quantity_update(auth_headers):
    ...
    log = db.query(AuditLog).filter(
        AuditLog.resource == "SKU SKU-101",
        AuditLog.action == "modify_quantity"
    ).first()
...
def test_rejected_status_transition(auth_headers):
    ...
    db = SessionLocal()
    from database.models import RiskScore, AuditLog
    ...
    assert log is not None
    assert "Changed status to Rejected" in log.detail
    db.close()
...
def test_decision_notes_retroduction_and_audit(auth_headers):
    ...
    matching_note = next((n for n in notes if n["note"] == "Verification test notes content"), None)
    ...
    resp_missing = client.get(
        "/api/enterprise/decisions/SKU-MISSING/notes",
        headers=auth_headers
    )
    assert resp_missing.status_code == 404
...
def test_reorder_safeguards_and_edge_cases(auth_headers):
    ...
    db = SessionLocal()
    from database.models import Product, Supplier, InventoryItem, Warehouse
    
    # Ensure test warehouse exists
    wh = db.query(Warehouse).first()
    if not wh:
        wh = Warehouse(name="Test Reorder Warehouse", location="Test Loc", capacity=1000.0)
        db.add(wh)
        db.commit()
    ...
    p_sup_present = Product(sku="SKU-TEST-SUP-PRES", name="Test Sup Present Prod", category="Beverages", base_price=10.0, unit_cost=5.0, supplier_id=sup1.id)
    db.add(p_sup_present)
    db.commit()
    inv_sup_present = InventoryItem(product_id=p_sup_present.id, warehouse_id=wh.id, current_stock=20.0)
    db.add(inv_sup_present)
    db.commit()

    # 2. Supplier missing
    p_sup_missing = Product(sku="SKU-TEST-SUP-MISS", name="Test Sup Missing Prod", category="Beverages", base_price=10.0, unit_cost=5.0, supplier_id=None)
    db.add(p_sup_missing)
    db.commit()
    inv_sup_missing = InventoryItem(product_id=p_sup_missing.id, warehouse_id=wh.id, current_stock=20.0)
    db.add(inv_sup_missing)
    db.commit()

    # 3. Unit cost missing
    p_cost_missing = Product(sku="SKU-TEST-COST-MISS", name="Test Cost Missing Prod", category="Beverages", base_price=10.0, unit_cost=None, supplier_id=sup1.id)
    db.add(p_cost_missing)
    db.commit()
    inv_cost_missing = InventoryItem(product_id=p_cost_missing.id, warehouse_id=wh.id, current_stock=20.0)
    db.add(inv_cost_missing)
    db.commit()

    # 4. Reorder point = 0 (low lead time and no sales/forecast)
    p_rop_zero = Product(sku="SKU-TEST-ROP-ZERO", name="Test ROP Zero Prod", category="Beverages", base_price=10.0, unit_cost=5.0, supplier_id=None, lead_time_days=0)
    db.add(p_rop_zero)
    db.commit()
    inv_rop_zero = InventoryItem(product_id=p_rop_zero.id, warehouse_id=wh.id, current_stock=10.0)
    db.add(inv_rop_zero)
    db.commit()

    # 5. Stock above reorder point (very high stock)
    p_above_rop = Product(sku="SKU-TEST-ABOVE", name="Test Above Prod", category="Beverages", base_price=10.0, unit_cost=5.0, supplier_id=sup1.id)
    db.add(p_above_rop)
    db.commit()
    inv_above_rop = InventoryItem(product_id=p_above_rop.id, warehouse_id=wh.id, current_stock=1000.0)
    db.add(inv_above_rop)
    db.commit()

    # 6. Stock below reorder point (stock is 0)
    p_below_rop = Product(sku="SKU-TEST-BELOW", name="Test Below Prod", category="Beverages", base_price=10.0, unit_cost=5.0, supplier_id=sup1.id)
    db.add(p_below_rop)
    db.commit()
    inv_below_rop = InventoryItem(product_id=p_below_rop.id, warehouse_id=wh.id, current_stock=0.0)
