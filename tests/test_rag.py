...
def test_copilot_chat_api_endpoint():
    ...
    login_resp = client.post("/api/auth/login", data={"username": "rag_tester", "password": "testpassword123"})
    token = login_resp.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    try:
        response = client.post("/api/enterprise/copilot/chat", json={"prompt": "What should I order?"}, headers=headers)
