...
class TestInventoryRisk:
    def test_high_risk_when_low_stock(self):
        risk = score_inventory_risk("SKU-X", [100.0] * 30, stock_on_hand=500, unit_cost=10)
        assert risk.risk_level == "High"

    def test_low_risk_when_well_stocked(self):
        # stock=450, 30-day forecast=300 → coverage fine, no overstock trigger
        risk = score_inventory_risk("SKU-X", [10.0] * 30, stock_on_hand=450, unit_cost=10)
        assert risk.risk_level == "Low"

    def test_revenue_at_risk_positive_for_stockout(self):
        risk = score_inventory_risk("SKU-X", [100.0] * 30, stock_on_hand=100, unit_cost=5)
        assert risk.revenue_at_risk > 0

    def test_no_revenue_risk_when_stocked(self):
        risk = score_inventory_risk("SKU-X", [10.0] * 30, stock_on_hand=9999, unit_cost=5)
        assert risk.revenue_at_risk == 0
    ...
    def test_reorder_qty_non_negative(self):
        risk = score_inventory_risk("SKU-X", [50.0] * 30, stock_on_hand=200, unit_cost=10)
    ...
    def test_days_of_cover_calculation(self):
        risk = score_inventory_risk("SKU-X", [10.0] * 30, stock_on_hand=200, unit_cost=5)
...
class TestPortfolioSummary:
    def _make_risk(self, level: str) -> InventoryRisk:
        return InventoryRisk(
            product_id="X", risk_level=level, risk_type="Stock-out",
            forecast_demand=100, stock_on_hand=50,
            recommended_reorder_qty=60, priority_score=10,
            financial_priority=10.0,
            revenue_at_risk=100.0, profit_at_risk=40.0, overstock_value=0,
            service_level=75.0, days_of_cover=5.0,
            forecast_confidence=90, expected_stockout_days=3.0,
            recommended_action="Reorder Immediately", root_causes=["test"],
            message="test",
        )

    def test_counts_high_risk(self):
        risks   = [self._make_risk("High"), self._make_risk("High"), self._make_risk("Low")]
...
class TestPortfolioRiskRanking:
    ...
    def test_each_result_has_required_keys(self, products):
        ...
        rankings = engine.rank(products)
        for r in rankings:
            assert all(k in r for k in ["sku", "risk_score", "risk", "stockout", "overstock", "revenue_risk"])
